...
- [🏗️ System Architecture](#architecture)
  ...
  - [⚙️ Application Layer](#ApplicationLayer)
  - [🗃️ Database Layer](#DatabaseLayer)
- [🔒 Seamless Auth0 Integration for User Authentication and Authorization](#Auth0)
  - [🔐 Securing Endpoints](#SecuringEndpoints)
  - [🛡️ Role-Based Access Control](#RBAC)
  - [🚧 Integrating Secure Endpoints with RBAC](#SecuringRBAC)
  - [🚪 Integrating Auth0 User Login](#Login)
- [🚫 Securing Pages from Unauthorized Access via URL Manipulation](#url)
- [✨ Additional Exciting functionalities ](#add)
- [🔗 Links](#Links)
...
In summary, this code sets up a middleware (`checkJwt`) applied to protect the API endpoints within the application using JSON Web Tokens (JWTs). This ensures that only authenticated and authorized users can access secured resources.
...
Within the UserContext.jsx file, I destructured the following objects by invoking the `useAuth0()` function imported from the `@auth0/auth0-react` package:
...
- The login page dynamically displays either the 'Go to Login' button when the user is already authenticated or the 'Logout' button when the user is not logged in. This behavior is determined by checking the value of the `myUser` state as shown in the code snippet below. This can also be achieved using the `isAuthenticated` state.
...
## Additional Exciting functionalities <a id="add"></a>
...
<p align="center">
  <Image
    alt={`Database schema`}
    src={`/static/images/projects/resource booking app/a.gif`}
    width={580}
    height={220}
    priority
  />
